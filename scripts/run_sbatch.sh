...
# for f in "x^2" "x^3"; do # run locally for debug
    ...
    sbatch --export=competitor=$competitor,resfolder0=$resfolder0,nlam=$nlam,nrep=$nrep,nfold=$nfold,one_se_rule=$one_se_rule,f=$f,timestamp=$timestamp,n=$n,use_snr=$use_snr --output nrep$nrep-nfold$nfold-nlam$nlam-1se_$one_se_rule-$competitor-$f-$timestamp-n$n-snr_$use_snr.log ex.job 
done
