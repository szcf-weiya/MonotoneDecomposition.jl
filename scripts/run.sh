...
# for f in "x^2" "x^3"; do # run locally for debug
    julia --project=.. ../examples/benchmark.jl $competitor $resfolder0 $nlam $nrep $nfold $one_se_rule $f $timestamp $n &> nrep$nrep-nfold$nfold-nlam$nlam-$one_se_rule-$competitor-$f-$timestamp-n$n.log &
done
